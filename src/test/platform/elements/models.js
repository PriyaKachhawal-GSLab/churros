...
const payload = require("core/tools").requirePayload(
  `${__dirname}/assets/element.elementmodel.payload.json`
);
...
suite.forPlatform("elements/models", opts, test => {
  ...
  after(() => {
    return globalModelId ? cloud.delete(`elements/${element.id}/models/${globalModelId}`) : null;
  });

  it("should support CRUD for models", () =>
    crudsObject(
      idUrl,
      schema,
      genObject({}),
      genObject({ createdDateName: "created_date" })
    ));
  ...
  it("should support upserting a model", () => {
    ...
    return cloud
      .put(`/elements/${element.key}/models`, genObject({}))
      .then(r => {
      ...
      .then(() => {
        ...
        object.id = modelId;
        return cloud.put(`/elements/${element.key}/models`, object);
      })
      .then(r => {
      ...
      .catch(e => {
        if (modelId) {
          cloud.delete(`/elements/${element.key}/models/${modelId}`);
        }
        throw e;
      });
  });
  ...
  it("should support CRUD for models with an associated resource", () => {
    return cloud
      .post(
        `/elements/${element.key}/models`,
        genObject({ resources: [{ id: resourceIds[0] }] })
      )
      .then(r => {
      ...
      .then(r =>
        cloud.put(
          `/elements/${element.key}/models/${r.body.id}`,
          genObject({
            resources: [{ id: resourceIds[0] }, { id: resourceIds[1] }]
          })
        )
      )
      .then(r => {
      ...
      .then(r =>
        cloud.put(
          `/elements/${element.key}/models/${r.body.id}`,
          genObject({ resources: [{ id: resourceIds[1] }] })
        )
      )
