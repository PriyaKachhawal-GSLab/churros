...
suite.forElement('documents', 'files', { payload: payload }, (test) => {
  let jpgFileBody, textFileBody, revisionId,
  ...
  it(`should allow POST /files/:id/thumbnails by providing folder id`, () => {
       ...
       return cloud.withOptions({ qs: { path: `/unknownType-${tools.randomStr('abcdefghijklmnopqrstuvwxyz1234567890', 10)}.gliffy` } }).postFile(test.api, textFile)
          .then(r => {
            textFileBody = r.body;
          ...
          .then(() => cloud.postFile(`${test.api}/${jpgFileBody.id}/thumbnails`, jpgFile))
          .then(r => {
            expect(r.body.properties).to.contain.key("thumbnailLink");
