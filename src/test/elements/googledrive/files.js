...
suite.forElement('documents', 'files', { payload: payload }, (test) => {
  ...
  before(() => cloud.withOptions({ qs: query }).postFile(test.api, jpgFile)
    ...
    .then(() => cloud.withOptions({ qs: { path: `/unknownType-${tools.randomStr('abcdefghijklmnopqrstuvwxyz1234567890', 10)}.gliffy` } }).postFile(test.api, textFile))
    .then(r => {
      textFileBody = r.body;
  ...
  it(`should allow POST /files/:id/thumbnails by providing folder id`, () => {
          return cloud.postFile(`${test.api}/${jpgFileBody.id}/thumbnails`, jpgFile)
            .then(r => expect(r.body.properties.thumbnailLink).not.to.be.null);
    });
});
